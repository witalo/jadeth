...
function CreateSales() {
    ...
    if (date === "") {
        toastr.warning('Ingrese la fecha de la venta')
        return false
    }
...
function SendSales(object) {
    console.log(object)
    let r = confirm('¿ESTA SEGURO DE PROCESAR LA VENTA?');
    if (r === true) {
        $.ajax({
    ...
    if (e.keyCode === 13) {
        ...
        if (order !== '') {
            ...
            $.ajax({
                ...
                dataType: 'json',
                type: 'GET',
                data: {'order': order, 'type': 'V'},
                success: async function (response) {
                    if (response.pk) {
                        $("#order").val(response.pk);
                        $("#client").val(response.person);

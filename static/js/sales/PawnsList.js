        ...
        renderResult: (result, props) => {
            return `                
                <li ${props} class="font-weight-bold">
                 <div class="text-white-50 h5 d-flex align-items-center">
                    ${'<i class="spinner-grow text-center align-self-center"></i>'} ${result.name}
                 </div>
...
function CreatePawns() {
    ...
    if (isNaN(provider) || provider === "") {
        provider = 0
        toastr.warning('Porfavor ingrese un cliente')
        return false
    }
    let date = $('#date_current').val()
    if (date === "") {
        toastr.warning('Ingrese la fecha a realizarse')
        return false
    }
...
function SendPawns(object) {
    console.log(object)
    let r = confirm('¿ESTA SEGURO DE REALIZAR LA OPERACION?');
    if (r === true) {
        $.ajax({
            url: '/sales/pawns_save/',
            dataType: 'json',
            type: 'POST',
...
function ShowModalPawns() {
    $.ajax({
        url: '/sales/modal_pawns/',
        dataType: 'json',
        type: 'GET',
        success: function (response) {
            if (response.success) {
                $('#modal-pawns').empty().html(response.form).modal('show');
            }
        },
        fail: function (response) {
